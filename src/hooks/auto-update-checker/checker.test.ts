      ...
      readMock.mockImplementation((p: string) => {
        if (p.includes('opencode.json')) {
          return JSON.stringify({
            plugin: ['file:///dev/oh-my-opencode-lite'],
          });
        }
  ...
  describe('findPluginEntry', () => {
    test('detects latest version entry', () => {
      ...
      readMock.mockImplementation(() =>
        JSON.stringify({
          plugin: ['oh-my-opencode-lite'],
        }),
      );

      const entry = findPluginEntry('/test');
      expect(entry).not.toBeNull();
      expect(entry?.entry).toBe('oh-my-opencode-lite');
    ...
    test('detects pinned version entry', () => {
      ...
      readMock.mockImplementation(() =>
        JSON.stringify({
          plugin: ['oh-my-opencode-lite@1.0.0'],
        }),
      );
