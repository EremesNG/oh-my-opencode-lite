...
import { writeTextFile } from '../../utils/file-io';
...
export function getCacheDir(): string {
  if (process.platform === 'win32') {
    ...
    return join(base, 'oh-my-opencode-lite', 'bin');
  }
...
export async function downloadAstGrep(
  ...
  if (!platformInfo) {
    console.error(
      `[oh-my-opencode-lite] Unsupported platform for ast-grep: ${platformKey}`,
    );
    return null;
  }
  ...
  console.log(`[oh-my-opencode-lite] Downloading ast-grep binary...`);
  ...
  try {
    ...
    console.log(`[oh-my-opencode-lite] ast-grep binary ready.`);

    return binaryPath;
  } catch (err) {
    console.error(
      `[oh-my-opencode-lite] Failed to download ast-grep: ${err instanceof Error ? err.message : err}`,
    );
    return null;
  }
}
